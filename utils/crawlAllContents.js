...
const crawlAllContents = (content, pageNum) => {
  ...
  dataDummy.forEach((row, idx) => {
    ...
    const data = { bsnsNm, bidNtceNoWithOrd, bidNtceDetailLink, bsnsType, bidNtceNm, ntceInstNm, dmInstNm, cntrctMthdNm, insertTime, dueTime, cmmnSpldmd, bid };
...
const valueByIdx = (contentList) => {
  ...
  for(let idx = 0; idx < 10; ++idx) {
    ...
    if(idx === 1 || idx === 7) { 
      ...
      metaData =  idx === 1 
              ? metaDir ? metaDir["href"] : "" 
              : metaDir ? metaDir.data.match(/\((.*?)\)/)[1]  : "";
    }
    else if (idx === 8 || idx === 9) {
